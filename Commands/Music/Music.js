...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('skip').setDescription('Saltar a la siguiente canción')
    ),
  async execute(interaction) {
    ...
    if (!interaction || !interaction.guild) {
      interaction.followUp('No puedes usar este comando en este canal.')
      return
    }
        ...
        case `stop`:
          {
            ...
            if (!musicData.isPlaying) {
              return interaction.followUp('No hay canciones reproduciéndose.')
            }
        ...
        case `skip`:
          {
            ...
            if (musicData.playlist.length === 0) {
              return interaction.followUp('No hay más canciones en la lista.')
            }
...
function createNowPlayingEmbed(
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      '**Al poner una cancion nueva se agregara a la lista de reproducción no se reproducirá inmediatamente**'
    )
    .addFields(
      {
        name: 'Canción',
...
async function editPlaylistMessage(interaction, embed, guildId) {
  ...
  try {
    ...
    if (message) {
      // Si el mensaje existe, edítalo con el nuevo embed
      await message.edit({ embeds: [embed] })
      return interaction.followUp(`Lista actualizada`)
    }
  } catch (error) {
...
async function sendPlaylistMessage(interaction, embed, guildId) {
  ...
  try {
    ...
    console.error('Error al enviar el mensaje de lista de reproducción:', error)
  }
}
